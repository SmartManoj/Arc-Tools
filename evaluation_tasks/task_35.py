...
def close_the_lock(grid: Grid):
    ...
    gray_objects = detect_objects(grid, required_color=Color.LIGHT_GRAY)
    ...
    for col in range(0, key_container.width, 2):
        ...
        for row in range(key_container.height-1):
            if key_container[row][col] == Color.LIGHT_GRAY.value:
                col_height += 1
        heights.append(col_height)

    lock_bites = [3, 4 , 4 , 3]
    max_lock_bite = max(lock_bites)
    bar_objects = detect_objects(grid, ignore_color=Color.LIGHT_GRAY)
    ...
    for bar_idx, bar_object in enumerate(bar_objects):
        ...
        move_object(bar_object, dx=0, dy=dy, grid=new_grid, fill_color=Color.LIGHT_GRAY)
